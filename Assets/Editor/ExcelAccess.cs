...
{
    //查询语言表
    public static List<AskItem> SelectMenuLang(int index)
    {
        string excelName = "PSQ.xlsx";
        DataRowCollection collect = ExcelAccess.ReadExcel(excelName, index);

        List<AskItem> menuArray = new List<AskItem>();
        for (int i = 1; i < collect.Count; i++)
        {
            if (collect[i][1].ToString() == "") continue;

            AskItem menu = new AskItem
            {
                problem = collect[i][0].ToString(),
                answer = collect[i][1].ToString(),
            };
            menuArray.Add(menu);
        }
        return menuArray;
